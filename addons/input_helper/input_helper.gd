...
func serialize_inputs_for_action(action: StringName) -> String:
    ...
    for input in inputs:
        if input is InputEventKey:
            ...
            action_inputs.append("joypad_button:%d|%d" % [input.device, input.button_index])
        elif input is InputEventJoypadMotion:
            action_inputs.append("joypad_motion:%d|%d|%f" % [input.device, input.axis, input.axis_value])
    ...
    for action_input in action_inputs:
        ...
        match input_type:
            ...
            "joypad_motion":
                ...
                joypad_motion_input.device = int(joypad_bits[0])
                joypad_motion_input.axis = int(joypad_bits[1])
                joypad_motion_input.axis_value = float(joypad_bits[2])
            ...
            "joypad_button":
                ...
                var joypad_bits = input_details.split("|")
                joypad_input.device = int(joypad_bits[0])
                joypad_input.button_index = int(joypad_bits[1])
